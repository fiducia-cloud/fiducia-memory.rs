...
//!
//! This module wires two merged implementations over one shared SeaORM pool:
//!
...
//!     [`crate::fusion`]).
//!
//! On boot the service applies the embedded unified migrations through SeaORM,
//! covering both durable `memory_claims` and the epistemic
...
/// Unified application state over a single shared SeaORM connection pool.
#[derive(Clone)]
struct AppState {
...
pub async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // One shared SeaORM connection pool feeds both persistence layers.
    let pg = PostgresMemory::connect(&database_url).await?;
    let durable = MemoryStore::new(pg.database().clone());
